-- 007_acl_sync_and_duplicate_safety.sql
-- 1. Use DISTINCT ON in dochub_sync_authorization_snapshot to prevent duplicate key violations
-- 2. Allow active organization members to read folder_acl_entries for client-side effective permission calculations
...
BEGIN
  ...
  -- 6. Replace ACL entries with DISTINCT ON safety to avoid duplicate key violations
  ...
  SELECT DISTINCT ON (
    ...
    CASE WHEN a->>'principalType' = 'user' THEN m.id ELSE g.id END
  )
    p_organization_id, a->>'resourceId',
    CASE a->>'principalType' WHEN 'user' THEN 'member' ELSE 'group' END,
...
-- Allow all active organization members and owners to read folder_acl_entries
DROP POLICY IF EXISTS "Owners can view ACL" ON public.folder_acl_entries;
DROP POLICY IF EXISTS "Members can view ACL" ON public.folder_acl_entries;
CREATE POLICY "Members can view ACL" ON public.folder_acl_entries FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM public.organization_members m
      WHERE m.organization_id = folder_acl_entries.organization_id
        AND m.user_id = auth.uid()
        AND m.status = 'active'
    )
    OR EXISTS (
      SELECT 1 FROM public.organizations o
      WHERE o.id = folder_acl_entries.organization_id
